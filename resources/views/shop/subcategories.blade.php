      ...
      <div class="col-lg-3 col-md-5">
        <div class="sidebar">
          <div class="sidebar__item">
            <h4>Kategori {{ $parent_cat->name }}</h4>
            <ul>
              @foreach($parent_cat->child as $subcat)
              <li class="subcategory {{ Request::is('shop/'.$subcat->id.'/'.$parent_cat->id.'/categories/'.str_replace(' ', '-', strtolower($subcat->name))) ? 'active' : '' }}">
                <a href="{{ route('shop.subcategories', ['id' => $subcat->id, 'slug' => str_replace(' ', '-', strtolower($subcat->name)), 'parent_id' => $subcat->parent->id ]) }}">{{ $subcat->name }}</a>
              </li>
              @endforeach
            </ul>
          </div>
